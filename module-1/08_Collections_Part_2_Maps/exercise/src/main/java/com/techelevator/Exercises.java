...
public class Exercises {
    ...
    public double isItOnSale(String itemNumber) {
        Map<String, Double> itemDiscount = new HashMap<>();
        ...
        if (itemNumber == null || !itemDiscount.containsKey(itemNumber.toUpperCase())) {
            return 0.00;
        }
    ...
    public Map<String, Integer> robPeterToPayPaul(Map<String, Integer> peterPaul) {
        ...
        if (peterMoney > 0 && paulMoney < 1000) {
            ...
            peterPaul.put("Paul", paulMoney + stolenMoney);
        }
    ...
    public Map<String, Integer> peterPaulPartnership(Map<String, Integer> peterPaul) {
        ...
        if (peterMoney >= 5000 && paulMoney >= 10000) {
            int splitMoneyPeter = peterMoney / 4;
            int splitMoneyPaul = paulMoney / 4;
            peterPaul.put("Peter", peterMoney - splitMoneyPeter);
            peterPaul.put("Paul", paulMoney - splitMoneyPaul);
            peterPaul.put("PeterPaulPartnership", splitMoneyPaul + splitMoneyPeter);
        }
        return peterPaul;
    }
    ...
    public Map<String, String> beginningAndEnding(String[] words) {
        ...
        for (String word : words) {
            characters.put(word.substring(0, 1), word.substring(word.length() - 1));
        }
        return characters;
    }
    ...
    public Map<Integer, Integer> integerCount(int[] ints) {
        ...
        for (Integer integer : ints) {
            if (numCount.containsKey(integer)) {
                int count = numCount.get(integer);
                count++;
                numCount.put(integer, count);
            } else {
                numCount.put(integer, 1);
            }
        }
        return numCount;
    }
    ...
    public Map<String, Integer> consolidateInventory(Map<String, Integer> mainWarehouse,
        ...
        Map<String, Integer> mergedWarehouse = new HashMap<>();

        for (String name : mainWarehouse.keySet()) {
            if (remoteWarehouse.containsKey(name)) {
                mergedWarehouse.put(name, mainWarehouse.get(name) + remoteWarehouse.get(name));
            } else {
                mergedWarehouse.put(name, mainWarehouse.get(name));
            }
        }
        for (String name : remoteWarehouse.keySet()) {
            if (!mainWarehouse.containsKey(name)) {
                mergedWarehouse.put(name, remoteWarehouse.get(name));
            }
        }
    ...
    public Map<String, Integer> last2Revisited(String[] words) {
        Map<String, Integer> lastTwoCounted = new HashMap<>();
        ...
        for (String word : words) {
            ...
            lastTwoCounted.put(word, count);
        }
        return lastTwoCounted;
    }
}
